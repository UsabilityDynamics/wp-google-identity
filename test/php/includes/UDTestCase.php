...
class UD_Plugin_WP_UnitTestCase extends WP_UnitTestCase {
  ...
  function setUp() {
    ...
    if( file_exists( $this->root_dir . '/wp-google-identity.php' ) ) {
      include_once( $this->root_dir . '/wp-google-identity.php' );
    }
    if( !class_exists( '\UsabilityDynamics\WPGI\Bootstrap' ) ) {
      $this->fail( 'Plugin is not available.' );
    }
    $this->instance = \UsabilityDynamics\WPGI\Bootstrap::get_instance();
  }
